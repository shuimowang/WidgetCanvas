...
# Switch to and show one named canvas; matching is case-insensitive
...
`--canvas` also accepts `-c` and `/canvas`. Canvas names are unique on the local machine. The library is shared by every canvas, and switching canvases does not affect detached widget windows.
